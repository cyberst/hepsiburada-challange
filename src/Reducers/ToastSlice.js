...
export const setToastStateAsync = action => dispatch => {
    dispatch(setToastState({ linkName: action.linkName, removedOrAdded: action.removedOrAdded }));
    setTimeout(() => {
        dispatch(setToastState({ linkName: "", removedOrAdded: true }));
    }, action.duration);
};

export const selectToastState = state => ({ linkName: state.toast.linkName, removedOrAdded: state.toast.removedOrAdded });
