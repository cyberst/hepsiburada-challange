...
export const { UpVote, DownVote, SetSortOptions, SetHoveredLinkId, RemoveLink, AddLink } = LinksSlice.actions;
...
export const selectLinkInfos = state => {
    return state.links.links.map((link) => (({ id, linkName, linkURL, vote }) => ({ id, linkName, linkURL, vote }))(link));
}
